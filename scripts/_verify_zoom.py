...
from src.pipeline import (load_mesh_points, render_model, register_rgbd,
                          RegConfig)
from src.nonrigid_rtw import rtw_optimize_region_zoom, _residual_map, \
    FFDLattice
...
# c0 投影像素
Pc0 = transform(T_rigid, c0.reshape(1, 3))[0]
u0 = int(np.clip(K[0, 0] * Pc0[0] / Pc0[2] + K[0, 2], 0, W - 1))
v0 = int(np.clip(K[1, 1] * Pc0[1] / Pc0[2] + K[1, 2], 0, H - 1))
print(f'凹痕中心像素=({u0},{v0})')

for zoom in [3.0, 5.0]:
    res = rtw_optimize_region_zoom(
        model, depth_obs, K, T_rigid, (u0, v0), zoom=zoom,
        grid=(10, 10, 10), lambda_mag=1e-5, lambda_sm=1e-4,
        lambda_p2p=1.0, focal_gamma=2.0, inlier_hi=30.0, inlier_lo=12.0,
        iters=400, lr=1e-2, device=device, verbose=True, seed=0)
    ...
    print(f'zoom={zoom}: rot={rot_w:.2f}° NN={dr.mean():.2f}mm '
